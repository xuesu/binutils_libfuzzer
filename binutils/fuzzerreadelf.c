...
char *program_name = "fuzzerreadelf";
...
static const char *get_symbol_version_string(Filedata *, bfd_boolean, const char *, unsigned long, unsigned,
         ...
         const char *reason)
{
    ...
    if ((size_t)size != size || (size_t)nmemb != nmemb || (size_t)amt != amt)
    {
        if (reason)
    ...
    if (archive_file_offset > filedata->file_size || offset > filedata->file_size - archive_file_offset || amt > filedata->file_size - archive_file_offset - offset)
    {
        if (reason)
...
{
    return filedata->file_header.e_machine == EM_IA_64 && filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_OPENVMS;
}
...
#ifdef BFD64
            if (filedata->file_header.e_machine == EM_MIPS && filedata->file_header.e_ident[EI_DATA] != ELFDATA2MSB)
            {
               ...
               accordingly.  */
                ...
                inf = (((inf & 0xffffffff) << 32) | ((inf >> 56) & 0xff) | ((inf >> 40) & 0xff00) | ((inf >> 24) & 0xff0000) | ((inf >> 8) & 0xff000000));
                relas[i].r_info = inf;
            }
#endif /* BFD64 */
        }
...
{
    return filedata->file_header.e_machine == EM_MSP430 /* Paranoia.  */
                 ...
                 bfd_boolean is_dynsym)
{
    ...
    for (i = 0; i < rel_size; i++)
    {
        ...
        if (filedata->file_header.e_machine == EM_ALPHA && rtype != NULL && streq(rtype, "R_ALPHA_LITUSE") && is_rela)
        {
        ...
        else if (symtab_index)
        {
            ...
            else
            {
                ...
                version_string = get_symbol_version_string(filedata, is_dynsym,
                                                           strtab, strtablen,
                                                           symtab_index,
                                                           psym,
                                                           &sym_info,
                ...
                if (psym->st_name == 0)
                {
                    ...
                    if (ELF_ST_TYPE(psym->st_info) == STT_SECTION)
                    {
                        if (psym->st_shndx < filedata->file_header.e_shnum)
                            ...
                            sec_name = "COMMON";
                        else if ((filedata->file_header.e_machine == EM_MIPS && psym->st_shndx == SHN_MIPS_SCOMMON) || (filedata->file_header.e_machine == EM_TI_C6000 && psym->st_shndx == SHN_TIC6X_SCOMMON))
                            sec_name = "SCOMMON";
                        else if (filedata->file_header.e_machine == EM_MIPS && psym->st_shndx == SHN_MIPS_SUNDEFINED)
                            sec_name = "SUNDEF";
                        else if ((filedata->file_header.e_machine == EM_X86_64 || filedata->file_header.e_machine == EM_L1OM || filedata->file_header.e_machine == EM_K1OM) && psym->st_shndx == SHN_X86_64_LCOMMON)
                            sec_name = "LARGE_COMMON";
                        else if (filedata->file_header.e_machine == EM_IA_64 && filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_HPUX && psym->st_shndx == SHN_IA_64_ANSI_COMMON)
                            sec_name = "ANSI_COM";
                        else if (is_ia64_vms(filedata) && psym->st_shndx == SHN_IA_64_VMS_SYMVEC)
                            sec_name = "VMS_SYMVEC";
                        else
                        {
        ...
        if (filedata->file_header.e_machine == EM_SPARCV9 && rtype != NULL && streq(rtype, "R_SPARC_OLO10"))
...
get_parisc_dynamic_type(unsigned long type)
{
    ...
    case DT_HP_DLD_HOOK:
        return "HP_DLD_HOOK";
    case DT_HP_UX10_INIT:
        return "HP_UX10_INIT";
    case DT_HP_UX10_INITSZ:
        return "HP_UX10_INITSZ";
    case DT_HP_PREINIT:
        return "HP_PREINIT";
    case DT_HP_PREINITSZ:
        return "HP_PREINITSZ";
    case DT_HP_NEEDED:
        return "HP_NEEDED";
    case DT_HP_TIME_STAMP:
        return "HP_TIME_STAMP";
    case DT_HP_CHECKSUM:
        return "HP_CHECKSUM";
    case DT_HP_GST_SIZE:
        return "HP_GST_SIZE";
    case DT_HP_GST_VERSION:
        return "HP_GST_VERSION";
    case DT_HP_GST_HASHVAL:
        return "HP_GST_HASHVAL";
    case DT_HP_EPLTREL:
        return "HP_GST_EPLTREL";
    case DT_HP_EPLTRELSZ:
        return "HP_GST_EPLTRELSZ";
    case DT_HP_FILTERED:
        return "HP_FILTERED";
    case DT_HP_FILTER_TLS:
        return "HP_FILTER_TLS";
    case DT_HP_COMPAT_FILTERED:
        return "HP_COMPAT_FILTERED";
    case DT_HP_LAZYLOAD:
        return "HP_LAZYLOAD";
    case DT_HP_BIND_NOW_COUNT:
...
get_ia64_dynamic_type(unsigned long type)
{
    ...
    case DT_IA_64_VMS_SUBTYPE:
        return "VMS_SUBTYPE";
    case DT_IA_64_VMS_IMGIOCNT:
        return "VMS_IMGIOCNT";
    case DT_IA_64_VMS_LNKFLAGS:
        return "VMS_LNKFLAGS";
    case DT_IA_64_VMS_VIR_MEM_BLK_SIZ:
        return "VMS_VIR_MEM_BLK_SIZ";
    case DT_IA_64_VMS_IDENT:
        return "VMS_IDENT";
    case DT_IA_64_VMS_NEEDED_IDENT:
        return "VMS_NEEDED_IDENT";
    case DT_IA_64_VMS_IMG_RELA_CNT:
        return "VMS_IMG_RELA_CNT";
    case DT_IA_64_VMS_SEG_RELA_CNT:
        return "VMS_SEG_RELA_CNT";
    case DT_IA_64_VMS_FIXUP_RELA_CNT:
        return "VMS_FIXUP_RELA_CNT";
    case DT_IA_64_VMS_FIXUP_NEEDED:
        return "VMS_FIXUP_NEEDED";
    case DT_IA_64_VMS_SYMVEC_CNT:
        return "VMS_SYMVEC_CNT";
    case DT_IA_64_VMS_XLATED:
        return "VMS_XLATED";
    case DT_IA_64_VMS_STACKSIZE:
    ...
    case DT_IA_64_VMS_LINKTIME:
        return "VMS_LINKTIME";
    case DT_IA_64_VMS_SEG_NO:
        return "VMS_SEG_NO";
    case DT_IA_64_VMS_SYMVEC_OFFSET:
        return "VMS_SYMVEC_OFFSET";
    case DT_IA_64_VMS_SYMVEC_SEG:
    ...
    case DT_IA_64_VMS_SYSVER_OFFSET:
        return "VMS_SYSVER_OFFSET";
    case DT_IA_64_VMS_IMG_RELA_OFF:
        return "VMS_IMG_RELA_OFF";
    case DT_IA_64_VMS_SEG_RELA_OFF:
...
get_solaris_dynamic_type(unsigned long type)
{
    ...
    case 0x60000014:
        return "SUNW_SYMSORT";
    case 0x60000015:
        return "SUNW_SYMSORTSZ";
    case 0x60000016:
        return "SUNW_TLSSORT";
    case 0x60000017:
        return "SUNW_TLSSORTSZ";
    case 0x60000018:
        return "SUNW_CAPINFO";
    case 0x60000019:
        return "SUNW_STRPAD";
    ...
    case 0x6000001b:
        return "SUNW_LDMACH";
    case 0x6000001d:
        return "SUNW_CAPCHAINENT";
    case 0x6000001f:
        return "SUNW_CAPCHAINSZ";
    case 0x60000021:
        return "SUNW_PARENT";
    case 0x60000023:
        return "SUNW_ASLR";
...
get_dynamic_type(Filedata *filedata, unsigned long type)
{
    ...
    default:
        ...
        else if (((type >= DT_LOOS) && (type <= DT_HIOS)) || (filedata->file_header.e_machine == EM_PARISC && (type >= OLD_DT_LOOS) && (type <= OLD_DT_HIOS)))
        {
            const char *result;
    ...
    if (e_flags)
    {
        ...
        case EM_CYGNUS_MEP:
            ...
            case EF_MEP_CPU_H1:
                ...
                break;
            default:
                strcat(buf, _(", <unknown MeP cpu type>"));
                break;
            }
            ...
            case EF_MEP_COP_NONE:
                break;
            case EF_MEP_COP_AVC:
                strcat(buf, ", AVC coprocessor");
                break;
            case EF_MEP_COP_AVC2:
                strcat(buf, ", AVC2 coprocessor");
                break;
            case EF_MEP_COP_FMAX:
                strcat(buf, ", FMAX coprocessor");
                break;
            case EF_MEP_COP_IVC2:
                strcat(buf, ", IVC2 coprocessor");
                break;
            default:
                strcat(buf, _("<unknown MeP copro type>"));
                break;
            }
        ...
        case EM_V800:
            ...
            for (e_flags &= 0xFFFF; e_flags; e_flags &= ~(e_flags & -e_flags))
            {
                ...
                case EF_RH850_FPU_DOUBLE:
                    strcat(buf, ", double precision FPU");
                    break;
                case EF_RH850_FPU_SINGLE:
                    strcat(buf, ", single precision FPU");
                    break;
                case EF_RH850_REGMODE22:
                    strcat(buf, ", regmode:22");
                    break;
                case EF_RH850_REGMODE32:
                    strcat(buf, ", regmode:23");
                    break;
        ...
        case EM_MIPS_RS3_LE:
            ...
            case E_MIPS_MACH_LS2E:
                strcat(buf, ", loongson-2e");
                break;
            case E_MIPS_MACH_LS2F:
                strcat(buf, ", loongson-2f");
                break;
        ...
        case EM_SH:
            ...
            case EF_SH2A:
                ...
                break;
            case EF_SH4_NOFPU:
                strcat(buf, ", sh4-nofpu");
                break;
            case EF_SH4A_NOFPU:
                strcat(buf, ", sh4a-nofpu");
                break;
            case EF_SH2A_NOFPU:
                strcat(buf, ", sh2a-nofpu");
                break;
            case EF_SH3_NOMMU:
                strcat(buf, ", sh3-nommu");
                break;
            case EF_SH4_NOMMU_NOFPU:
                strcat(buf, ", sh4-nommu-nofpu");
                break;
            case EF_SH2A_SH4_NOFPU:
                strcat(buf, ", sh2a-nofpu-or-sh4-nommu-nofpu");
                break;
            case EF_SH2A_SH3_NOFPU:
                strcat(buf, ", sh2a-nofpu-or-sh3-nommu");
                break;
            case EF_SH2A_SH4:
                strcat(buf, ", sh2a-or-sh4");
                break;
            case EF_SH2A_SH3E:
                strcat(buf, ", sh2a-or-sh3e");
                break;
            default:
                strcat(buf, _(", unknown ISA"));
                break;
    ...
    default:
        if (osabi >= 64)
            ...
            case EM_TI_C6000:
                switch (osabi)
                {
                case ELFOSABI_C6000_ELFABI:
                    return _("Bare-metal C6000");
                case ELFOSABI_C6000_LINUX:
                    return "Linux C6000";
                default:
                    break;
                }
...
get_v850_section_type_name(unsigned int sh_type)
{
    ...
    case SHT_RENESAS_IOP:
        return "RENESAS IOP";
    case SHT_RENESAS_INFO:
        return "RENESAS INFO";
    default:
        return NULL;
    }
            ...
            default:
                ...
                else
                {
                    ...
                    case SHT_GNU_INCREMENTAL_INPUTS:
                        result = "GNU_INCREMENTAL_INPUTS";
                        break;
                    case SHT_GNU_ATTRIBUTES:
                        result = "GNU_ATTRIBUTES";
                        break;
                    case SHT_GNU_HASH:
                        result = "GNU_HASH";
                        break;
...
process_file_header(Filedata *filedata)
{
    ...
    if (do_header)
    {
        ...
        printf(_("  Number of program headers:         %u"),
               header->e_phnum);
        if (filedata->section_headers != NULL && header->e_phnum == PN_XNUM && filedata->section_headers[0].sh_info != 0)
        {
        ...
        printf(_("  Section header string table index: %u"),
               header->e_shstrndx);
        if (filedata->section_headers != NULL && header->e_shstrndx == (SHN_XINDEX & 0xffff))
        {
    ...
    if (filedata->section_headers != NULL)
    {
        if (header->e_phnum == PN_XNUM && filedata->section_headers[0].sh_info != 0)
            header->e_phnum = filedata->section_headers[0].sh_info;
...
get_program_headers(Filedata *filedata)
{
    ...
    if (filedata->file_header.e_phnum * (is_32bit_elf ? sizeof(Elf32_External_Phdr) : sizeof(Elf64_External_Phdr)) >= filedata->file_size)
    {
...
process_program_headers(Filedata *filedata)
{
	  ...
	  if (previous_load
        ...
        case PT_PHDR:
            ...
            if (filedata->file_header.e_machine != EM_PARISC)
            {
                ...
                for (j = 1; j < filedata->file_header.e_phnum; j++)
                {
                    ...
                    if (load->p_type == PT_LOAD && load->p_offset <= segment->p_offset && (load->p_offset + load->p_filesz >= segment->p_offset + segment->p_filesz) && load->p_vaddr <= segment->p_vaddr && (load->p_vaddr + load->p_filesz >= segment->p_vaddr + segment->p_filesz))
                        break;
                }
        ...
        case PT_DYNAMIC:
            ...
            if (filedata->section_headers != NULL)
            {
                ...
                if (dynamic_addr < segment->p_offset || dynamic_addr > segment->p_offset + segment->p_filesz)
            ...
            /* PR binutils/17512: Avoid corrupt dynamic section info in the
               ...
               sections).  */
            if (dynamic_addr > filedata->file_size || dynamic_size > filedata->file_size - dynamic_addr)
            {
    ...
    if (do_segments && filedata->section_headers != NULL && filedata->string_table != NULL)
    {
...
offset_from_vma(Filedata *filedata, bfd_vma vma, bfd_size_type size)
{
         ...
         ++seg)
    {
        ...
        if (vma >= (seg->p_vaddr & -seg->p_align) && vma + size <= seg->p_vaddr + seg->p_filesz)
...
get_elf_section_flags(Filedata *filedata, bfd_vma sh_flags)
{
    ...
    while (sh_flags)
    {
        ...
        else
        {
            ...
            default:
                if ((filedata->file_header.e_machine == EM_X86_64 || filedata->file_header.e_machine == EM_L1OM || filedata->file_header.e_machine == EM_K1OM) && flag == SHF_X86_64_LARGE)
                    *p = 'l';
                else if (filedata->file_header.e_machine == EM_ARM && flag == SHF_ARM_PURECODE)
                    *p = 'y';
                else if (filedata->file_header.e_machine == EM_PPC && flag == SHF_PPC_VLE)
                    *p = 'v';
                else if (flag & SHF_MASKOS)
                {
                    *p = 'o';
...
process_section_headers(Filedata *filedata)
{
    ...
    if (filedata->file_header.e_shstrndx != SHN_UNDEF && filedata->file_header.e_shstrndx < filedata->file_header.e_shnum)
    {
    ...
    case EM_MIPS_RS3_LE:
       ...
       the official LP64 form is being used.  */
        if ((filedata->file_header.e_flags & EF_MIPS_ABI) == E_MIPS_ABI_EABI64 && find_section(filedata, ".gcc_compiled_long32") == NULL)
            eh_addr_size = 8;
        break;
...
#define CHECK_ENTSIZE(section, i, type)                             \
         ...
         i++, section++)
    {
        ...
        else if ((do_debugging || do_debug_info || do_debug_abbrevs || do_debug_lines || do_debug_pubnames || do_debug_pubtypes || do_debug_aranges || do_debug_frames || do_debug_macinfo || do_debug_str || do_debug_loc || do_debug_ranges || do_debug_addr || do_debug_cu_index || do_debug_links) && (const_strneq(name, ".debug_") || const_strneq(name, ".zdebug_")))
        {
            if (name[1] == 'z')
        ...
        /* Linkonce section to be combined with .debug_info at link time.  */
            ...
            request_dump_bynumber(filedata, i, DEBUG_DUMP);
        else if (do_gdb_index && (streq(name, ".gdb_index") || streq(name, ".debug_names")))
            request_dump_bynumber(filedata, i, DEBUG_DUMP);
        ...
        else if ((do_debugging || do_trace_info || do_trace_abbrevs || do_trace_aranges) && const_strneq(name, ".trace_"))
        {
            ...
            if (do_debugging || (do_trace_info && streq(name, "info")) || (do_trace_abbrevs && streq(name, "abbrev")) || (do_trace_aranges && streq(name, "aranges")))
                request_dump_bynumber(filedata, i, DEBUG_DUMP);
        }
        else if ((do_debugging || do_debug_links) && (const_strneq(name, ".gnu_debuglink") || const_strneq(name, ".gnu_debugaltlink")))
            request_dump_bynumber(filedata, i, DEBUG_DUMP);
    }
         ...
         i++, section++)
    {
        ...
        case SHT_REL:
        case SHT_RELA:
            if (section->sh_link == 0 && (filedata->file_header.e_type == ET_EXEC || filedata->file_header.e_type == ET_DYN))
        ...
        case SHT_GNU_HASH:
        case SHT_GNU_versym:
            if (section->sh_link == 0 || section->sh_link >= filedata->file_header.e_shnum || (filedata->section_headers[section->sh_link].sh_type != SHT_SYMTAB && filedata->section_headers[section->sh_link].sh_type != SHT_DYNSYM))
        ...
        case SHT_GNU_verdef:
        case SHT_GNU_LIBLIST:
            if (section->sh_link == 0 || section->sh_link >= filedata->file_header.e_shnum || filedata->section_headers[section->sh_link].sh_type != SHT_STRTAB)
     ...
     section.  */
	  if (section->sh_type < SHT_LOOS && section->sh_link != 0)
        ...
        case SHT_REL:
        case SHT_RELA:
            if (section->sh_info == 0 && (filedata->file_header.e_type == ET_EXEC || filedata->file_header.e_type == ET_DYN))
                /* Dynamic relocations apply to segments, so they do not
                   need to specify the section they relocate.  */
                break;
            if (section->sh_info == 0 || section->sh_info >= filedata->file_header.e_shnum || (filedata->section_headers[section->sh_info].sh_type != SHT_PROGBITS && filedata->section_headers[section->sh_info].sh_type != SHT_NOBITS && filedata->section_headers[section->sh_info].sh_type != SHT_NOTE && filedata->section_headers[section->sh_info].sh_type != SHT_INIT_ARRAY && filedata->section_headers[section->sh_info].sh_type != SHT_FINI_ARRAY && filedata->section_headers[section->sh_info].sh_type != SHT_PREINIT_ARRAY
        ...
        default:
            ...
            else if (section->sh_type < SHT_LOOS && (section->sh_flags & SHF_GNU_MBIND) == 0 && section->sh_info != 0)
                warn(_("[%2u]: Unexpected value (%u) in info field.\n"),
                     i, section->sh_info);
            break;
        }
        ...
        if (section->sh_size > filedata->file_size && section->sh_type != SHT_NOBITS && section->sh_type != SHT_NULL && section->sh_type < SHT_LOOS)
  ...
  C (compressed), x (unknown), o (OS specific), E (exclude),\n  "));
        if (filedata->file_header.e_machine == EM_X86_64 || filedata->file_header.e_machine == EM_L1OM || filedata->file_header.e_machine == EM_K1OM)
            printf(_("l (large), "));
...
process_section_groups(Filedata *filedata)
{
         ...
         i++, section++)
    {
        if (section->sh_type == SHT_GROUP)
        {
            ...
            if (section->sh_link >= filedata->file_header.e_shnum || ((sec = filedata->section_headers + section->sh_link)->sh_type != SHT_SYMTAB))
            {
            ...
            {
                if (sym->st_shndx == 0 || sym->st_shndx >= filedata->file_header.e_shnum)
                {
                    error(_("Bad sh_info in group section `%s'\n"), name);
                    continue;
                }
            ...
            else
            {
                ...
                else if (strtab_sec != (sec = filedata->section_headers + symtab_sec->sh_link))
                {
                    strtab_sec = sec;
                    if (strtab)
                        free(strtab);
                             ...
                             unsigned int strtab_sz)
{
    ...
    printf(_("Seg Offset           Type                             SymVec DataType\n"));
    ...
    else
    {
             ...
             i++, section++)
        {
            ...
            if (rel_size)
            {
                ...
                if (section->sh_link != 0 && section->sh_link < filedata->file_header.e_shnum)
                {
                    ...
                    if (symsec->sh_link != 0 && symsec->sh_link < filedata->file_header.e_shnum)
                    {
                        ...
                        bfd_vma *offset)
{
    ...
    while (beg < end)
    {
        ...
        if (sym->st_name != 0 && (addr.section == SHN_UNDEF || addr.section == sym->st_shndx) && addr.offset >= value && addr.offset - value < dist)
        {
            best = sym;
...
dump_ia64_unwind(Filedata *filedata, struct ia64_unw_aux_info *aux)
{
    ...
    for (tp = aux->table; tp < aux->table + aux->table_len; ++tp)
    {
        ...
        if (offset >= aux->info_size || aux->info_size - offset < 8)
        {
                        ...
                        Elf_Internal_Shdr *sec)
{
    ...
    if (filedata->file_header.e_phnum)
    {
             ...
             ++seg)
        {
            ...
            if (sec->sh_addr >= seg->p_vaddr && (sec->sh_addr + sec->sh_size <= seg->p_vaddr + seg->p_memsz))
            {
    ...
    for (tp = table; tp <= table + size - (3 * eh_addr_size); ++tep)
    {
        ...
        tep->start.offset = byte_get(tp, eh_addr_size);
        tp += eh_addr_size;
        tep->end.offset = byte_get(tp, eh_addr_size);
        tp += eh_addr_size;
        tep->info.offset = byte_get(tp, eh_addr_size);
        tp += eh_addr_size;
    ...
    for (relsec = filedata->section_headers;
         ...
         ++relsec)
    {
        if (relsec->sh_type != SHT_RELA || relsec->sh_info >= filedata->file_header.e_shnum || filedata->section_headers + relsec->sh_info != sec)
            continue;
    ...
    while (unwcount-- > 0)
    {
       ...
       sections so the loop above should never fail.  */
        ...
        if ((unwsec->sh_flags & SHF_GROUP) != 0)
        {
            ...
            if (section_headers_groups == NULL || section_headers_groups[i] == NULL)
                i = filedata->file_header.e_shnum;
            else
            {
        ...
        else if (strneq(SECTION_NAME(unwsec), ELF_STRING_ia64_unwind_once, len))
        {
            ...
            for (i = 0, sec = filedata->section_headers; i < filedata->file_header.e_shnum;
                 ++i, ++sec)
                if (strneq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info_once, len2) && streq(SECTION_NAME(sec) + len2, suffix))
                    break;
        }
        else
        {
            ...
            for (i = 0, sec = filedata->section_headers; i < filedata->file_header.e_shnum;
                 ++i, ++sec)
                if (strneq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info, len2) && streq(SECTION_NAME(sec) + len2, suffix))
                    break;
        }
        ...
        else
        {
            ...
            if (slurp_ia64_unwind_table(filedata, &aux, unwsec) && aux.table_len > 0)
                        ...
                        bfd_vma *sym_name)
{
    ...
    if (sec != arm_sec->sec)
    {
        ...
        for (relsec = filedata->section_headers;
             ...
             ++relsec)
        {
            if (relsec->sh_info >= filedata->file_header.e_shnum || filedata->section_headers + relsec->sh_info != sec
                /* PR 15745: Check the section type as well.  */
                || (relsec->sh_type != SHT_REL && relsec->sh_type != SHT_RELA))
                continue;
    ...
    if (/* PR 21343 */ /* PR 18879 */
            sec->sh_size < 4 ||
        word_offset > (sec->sh_size - 4) || ((bfd_signed_vma)word_offset) < 0)
        return FALSE;
                  ...
                  struct arm_section *data_arm_sec)
{
    ...
    if ((word & 0x80000000) == 0)
    {
        ...
        if (fn == 0 && addr.section == SHN_UNDEF && addr.offset == 0 && sym_name != (bfd_vma)-1 && sym_name < aux->strtab_size)
        {
       ...
       words.  */
        if (procname != NULL && (const_strneq(procname, "__gcc_personality_v0") || const_strneq(procname, "__gxx_personality_v0") || const_strneq(procname, "__gcj_personality_v0") || const_strneq(procname, "__gnu_objc_personality_v0")))
        {
            remaining = 0;
            more_words = 1;
            ADVANCE;
    ...
    else
    {
       ...
       An exception-handling table entry for the compact model looks like:
        ...
        if (filedata->file_header.e_machine == EM_ARM && (word & 0x70000000))
        {
    ...
    for (i = 0; i < exidx_len; i++)
    {
       ...
       An index table entry consists of 2 words.
        ...
        else
        {
            ...
            if (entry_addr.section != SHN_UNDEF && entry_addr.section < filedata->file_header.e_shnum)
            {
                table_sec = filedata->section_headers + entry_addr.section;
                table_offset = entry_addr.offset;
                /* PR 18879 */
                if (table_offset > table_sec->sh_size || ((bfd_signed_vma)table_offset) < 0)
                {
         ...
         i++, section++)
    {
        ...
        case SHT_GNU_verdef:
        {
            ...
            for (idx = cnt = 0; cnt < section->sh_info; ++cnt)
            {
                ...
                {
                    if (aux.vda_next < sizeof(*eaux) && !(j == ent.vd_cnt - 1 && aux.vda_next == 0))
                    {
                ...
                if (ent.vd_next < sizeof(*edef) && !(cnt == section->sh_info - 1 && ent.vd_next == 0))
                {
        ...
        case SHT_GNU_verneed:
        {
            ...
            for (idx = cnt = 0; cnt < section->sh_info; ++cnt)
            {
                ...
                for (j = 0, isum = idx + ent.vn_aux; j < ent.vn_cnt; ++j)
                {
                    ...
                    if (aux.vna_next < sizeof(*eaux) && !(j == ent.vn_cnt - 1 && aux.vna_next == 0))
                    {
                ...
                if (ent.vn_next < sizeof(*entry) && !(cnt == section->sh_info - 1 && ent.vn_next == 0))
                {
                ...
                for (j = 0; (j < 4) && (cnt + j) < total; ++j)
                    ...
                    default:
                        ...
                        if (version_info[DT_VERSIONTAGIDX(DT_VERNEED)])
                        {
                            ...
                            offset = offset_from_vma(filedata, version_info[DT_VERSIONTAGIDX(DT_VERNEED)],
                            ...
                            do
                            {
                                ...
                                do
                                {
                                    ...
                                    a_off += ivna.vna_next;
                                } while (ivna.vna_other != data[cnt + j] && ivna.vna_next != 0);
                        ...
                        if (data[cnt + j] != 0x8001 && version_info[DT_VERSIONTAGIDX(DT_VERDEF)])
                        {
                            Elf_Internal_Verdef ivd;
                            Elf_External_Verdef evd;
                            unsigned long offset;

                            offset = offset_from_vma(filedata, version_info[DT_VERSIONTAGIDX(DT_VERDEF)],
                                                     sizeof evd);
...
get_symbol_binding(Filedata *filedata, unsigned int binding)
{
    ...
    default:
        ...
        {
            if (binding == STB_GNU_UNIQUE && filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_GNU)
                return "UNIQUE";
...
get_symbol_type(Filedata *filedata, unsigned int type)
{
    ...
    default:
        ...
        else if (type >= STT_LOOS && type <= STT_HIOS)
        {
            ...
            if (type == STT_GNU_IFUNC && (filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_GNU || filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_FREEBSD))
                return "IFUNC";
...
get_solaris_symbol_visibility(unsigned int visibility)
{
    ...
    case 6:
        return "ELIMINATE";
    default:
        return get_symbol_visibility(visibility);
    }
}
...
get_symbol_index_type(Filedata *filedata, unsigned int type)
{
    ...
    case SHN_COMMON:
        return "COM";
    default:
        if (type == SHN_IA_64_ANSI_COMMON && filedata->file_header.e_machine == EM_IA_64 && filedata->file_header.e_ident[EI_OSABI] == ELFOSABI_HPUX)
            return "ANSI_COM";
        else if ((filedata->file_header.e_machine == EM_X86_64 || filedata->file_header.e_machine == EM_L1OM || filedata->file_header.e_machine == EM_K1OM) && type == SHN_X86_64_LCOMMON)
            return "LARGE_COM";
        else if ((type == SHN_MIPS_SCOMMON && filedata->file_header.e_machine == EM_MIPS) || (type == SHN_TIC6X_SCOMMON && filedata->file_header.e_machine == EM_TI_C6000))
            return "SCOM";
        else if (type == SHN_MIPS_SUNDEFINED && filedata->file_header.e_machine == EM_MIPS)
            return "SUND";
...
get_dynamic_data(Filedata *filedata, bfd_size_type number, unsigned int ent_size)
{
    ...
    if (sizeof(size_t) < sizeof(bfd_size_type) && (bfd_size_type)((size_t)number) != number)
    {
                          ...
                          unsigned short *vna_other)
{
    ...
    if (!is_dynsym || version_info[DT_VERSIONTAGIDX(DT_VERSYM)] == 0)
        return NULL;
    ...
    *sym_info = (vers_data & VERSYM_HIDDEN) != 0 ? symbol_hidden : symbol_public;
    ...
    if (psym->st_shndx != SHN_UNDEF && vers_data != 0x8001 && version_info[DT_VERSIONTAGIDX(DT_VERDEF)])
    {
        Elf_Internal_Verdef ivd;
        Elf_Internal_Verdaux ivda;
        Elf_External_Verdaux evda;
    ...
    if (version_info[DT_VERSIONTAGIDX(DT_VERNEED)])
    {
        ...
        else if ((max_vd_ndx || (vers_data & VERSYM_VERSION) != 1) && (vers_data & VERSYM_VERSION) > max_vd_ndx)
            return _("<corrupt>");
    }
    return NULL;
}
...
process_symbol_table(Filedata *filedata)
{
    ...
    if (dynamic_info[DT_HASH] && (do_histogram || (do_using_dynamic && !do_dyn_syms && dynamic_strings != NULL)))
    {
        unsigned char nb[8];
        unsigned char nc[8];
        unsigned int hash_ent_size = 4;

        if ((filedata->file_header.e_machine == EM_ALPHA || filedata->file_header.e_machine == EM_S390 || filedata->file_header.e_machine == EM_S390_OLD) && filedata->file_header.e_ident[EI_CLASS] == ELFCLASS64)
            hash_ent_size = 8;

        if (fseek(filedata->handle,
                  (archive_file_offset + offset_from_vma(filedata, dynamic_info[DT_HASH],
                                                         sizeof nb + sizeof nc)),
                  SEEK_SET))
        {
    ...
    if (dynamic_info_DT_GNU_HASH && (do_histogram || (do_using_dynamic && !do_dyn_syms && dynamic_strings != NULL)))
    {
        unsigned char nb[16];
        ...
        if (fseek(filedata->handle,
                  (archive_file_offset + offset_from_vma(filedata, dynamic_info_DT_GNU_HASH,
                                                         sizeof nb)),
                  SEEK_SET))
        {
            error(_("Unable to seek to start of dynamic information\n"));
        ...
        if (fseek(filedata->handle,
                  (archive_file_offset + offset_from_vma(filedata, buckets_vma + 4 * (ngnubuckets + maxchain), 4)),
                  SEEK_SET))
        {
            error(_("Unable to seek to start of dynamic information\n"));
            goto no_gnu_hash;
        ...
        if (fseek(filedata->handle,
                  (archive_file_offset + offset_from_vma(filedata, buckets_vma + 4 * ngnubuckets, 4)),
                  SEEK_SET))
        {
            error(_("Unable to seek to start of dynamic information\n"));
            goto no_gnu_hash;
    ...
    if ((dynamic_info[DT_HASH] || dynamic_info_DT_GNU_HASH) && do_syms && do_using_dynamic && dynamic_strings != NULL && dynamic_symbols != NULL)
    {
        unsigned long hn;
    ...
    else if ((do_dyn_syms || (do_syms && !do_using_dynamic)) && filedata->section_headers != NULL)
    {
             ...
             i++, section++)
        {
            ...
            for (si = 0, psym = symtab; si < num_syms; si++, psym++)
            {
                ...
                version_string = get_symbol_version_string(filedata,
                ...
                if (ELF_ST_BIND(psym->st_info) == STB_LOCAL && si >= section->sh_info
...
process_syminfo(Filedata *filedata ATTRIBUTE_UNUSED)
{
    ...
    if (dynamic_syminfo == NULL || !do_dynamic)
    ...
    for (i = 0; i < dynamic_syminfo_nent; ++i)
    {
        ...
        case SYMINFO_BT_PARENT:
            ...
            break;
        default:
            if (dynamic_syminfo[i].si_boundto > 0 && dynamic_syminfo[i].si_boundto < dynamic_nent && VALID_DYNAMIC_NAME(dynamic_section[dynamic_syminfo[i].si_boundto].d_un.d_val))
            {
    ...
    case EM_MSP430_OLD:
    {
        ...
        handle_sym_diff:
            if (saved_sym != NULL)
            {
                ...
                {
                    value = reloc->r_addend + (symtab[sym_index].st_value - saved_sym->st_value);
...
is_32bit_abs_reloc(Filedata *filedata, unsigned int reloc_type)
{
    ...
    case EM_AARCH64:
        return (reloc_type == 258 || reloc_type == 1); /* R_AARCH64_ABS32 || R_AARCH64_P32_ABS32 */
    case EM_BPF:
                  ...
                  unsigned long *num_relocs_return)
{
         ...
         ++relsec)
    {
        ...
        if ((relsec->sh_type != SHT_RELA && relsec->sh_type != SHT_REL) || relsec->sh_info >= filedata->file_header.e_shnum || filedata->section_headers + relsec->sh_info != section || relsec->sh_size == 0 || relsec->sh_link >= filedata->file_header.e_shnum)
            continue;
        ...
        for (rp = relocs; rp < relocs + num_relocs; ++rp)
        {
            ...
            if (target_specific_reloc_handling(filedata, rp, start, end, symtab, num_syms))
                ...
                continue;
            else if (is_32bit_abs_reloc(filedata, reloc_type) || is_32bit_pcrel_reloc(filedata, reloc_type))
                reloc_size = 4;
            else if (is_64bit_abs_reloc(filedata, reloc_type) || is_64bit_pcrel_reloc(filedata, reloc_type))
                reloc_size = 8;
            else if (is_24bit_abs_reloc(filedata, reloc_type))
                reloc_size = 3;
            else if (is_16bit_abs_reloc(filedata, reloc_type))
                reloc_size = 2;
            else if (is_8bit_abs_reloc(filedata, reloc_type) || is_6bit_abs_reloc(filedata, reloc_type))
                reloc_size = 1;
            ...
            if (sym != symtab && ELF_ST_TYPE(sym->st_info) != STT_COMMON && ELF_ST_TYPE(sym->st_info) > STT_SECTION)
            {
            ...
            /* R_XTENSA_32, R_PJ_DATA_DIR32 and R_D30V_32_NORMAL are
               partial_inplace.  */
            if (!is_rela || (filedata->file_header.e_machine == EM_XTENSA && reloc_type == 1) || ((filedata->file_header.e_machine == EM_PJ || filedata->file_header.e_machine == EM_PJ_OLD) && reloc_type == 1) || ((filedata->file_header.e_machine == EM_D30V || filedata->file_header.e_machine == EM_CYGNUS_D30V) && reloc_type == 12) || reloc_inplace)
            {
            ...
            if (is_32bit_pcrel_reloc(filedata, reloc_type) || is_64bit_pcrel_reloc(filedata, reloc_type))
            {
            ...
            else if (is_6bit_abs_reloc(filedata, reloc_type) || is_6bit_inplace_sub_reloc(filedata, reloc_type))
            {
                if (reloc_subtract)
                    addend -= sym->st_value;
                else
...
dump_section_as_strings(Elf_Internal_Shdr *section, Filedata *filedata)
{
    ...
    if (decompress_dumps)
    {
        ...
        {
            Elf_Internal_Chdr chdr;
            unsigned int compression_header_size = get_compression_header(&chdr, (unsigned char *)start,
                                                                          num_bytes);
                      ...
                      bfd_boolean relocate)
{
    ...
    if (decompress_dumps)
    {
        ...
        {
            Elf_Internal_Chdr chdr;
            unsigned int compression_header_size = get_compression_header(&chdr, start, section_size);
...
get_build_id(void *data)
{
         ...
         i++, shdr++)
    {
        ...
        if (!is_ia64_vms(filedata))
        {
            ...
            inote.descdata = ((char *)enote + ELF_NOTE_DESC_OFFSET(inote.namesz, align));
            inote.descpos = offset + (inote.descdata - (char *)enote);
            next = ((char *)enote + ELF_NOTE_NEXT_OFFSET(inote.namesz, inote.descsz, align));
        }
        else
        {
        ...
        if ((size_t)(inote.descdata - inote.namedata) < inote.namesz || (size_t)(inote.descdata - inote.namedata) > data_remaining || (size_t)(next - inote.descdata) < inote.descsz || ((size_t)(next - inote.descdata) > data_remaining - (size_t)(inote.descdata - inote.namedata)))
        {
        ...
        if (inote.namesz > 0 && const_strneq(inote.namedata, "GNU") && inote.type == NT_GNU_BUILD_ID)
        {
            unsigned long j;
            char *build_id;
...
load_debug_section(enum dwarf_section_display_enum debug, void *data)
{
    ...
    if (filedata->string_table == NULL && filedata->file_header.e_shstrndx != SHN_UNDEF && filedata->file_header.e_shstrndx < filedata->file_header.e_shnum)
    {
...
display_debug_section(int shndx, Elf_Internal_Shdr *section, Filedata *filedata)
{
    ...
    for (i = 0; i < max; i++)
    {
        ...
        if (streq(sec->uncompressed_name, name) || (id == line && const_strneq(name, ".debug_line.")) || streq(sec->compressed_name, name))
        {
        ...
        case 0:
            ...
            case 7: /* Tag_CPU_arch_profile.  */
                ...
                case 0:
                    ...
                    break;
                case 'A':
                    printf(_("Application\n"));
                    break;
                case 'R':
                    printf(_("Realtime\n"));
                    break;
                case 'M':
                    printf(_("Microcontroller\n"));
                    break;
                case 'S':
                    printf(_("Application or Realtime\n"));
                    break;
                default:
                    printf("??? (%d)\n", val);
                    break;
            ...
            case 25: /* Tag_align_preserved.  */
                ...
                case 0:
                    ...
                    break;
                case 1:
                    printf(_("8-byte, except leaf SP\n"));
                    break;
                case 2:
                    printf(_("8-byte\n"));
                    break;
...
process_mips_specific(Filedata *filedata)
{
    ...
    if (dynamic_section == NULL)
    {
        ...
        if (sect != NULL)
        {
            ...
            /* In a dynamic binary GOT[0] is reserved for the dynamic
               ...
               before fetching it.  */
            if (data && data + ent - pltgot + addr_size <= data_end && byte_get(data + ent - pltgot, addr_size) == 0)
            {
               ...
               PR 21344: Check for the entry being fully available
               before fetching it.  */
                if (data && data + ent - pltgot + addr_size <= data_end && (byte_get(data + ent - pltgot, addr_size) >> (addr_size * 8 - 1)) != 0)
                {
                    ent = print_mips_got_entry(data, pltgot, ent, data_end);
                    printf("\n");
                    if (ent == (bfd_vma)-1)
    ...
    for (entry = dynamic_section;
        ...
        {
        case DT_MIPS_LIBLIST:
            liblist_offset = offset_from_vma(filedata, entry->d_un.d_val,
        ...
        case DT_MIPS_OPTIONS:
            ...
            break;
        case DT_MIPS_CONFLICT:
            conflicts_offset = offset_from_vma(filedata, entry->d_un.d_val,
    ...
    if (options_offset != 0)
    {
        ...
        if (eopt)
        {
            ...
            while (offset <= sect->sh_size - sizeof(*eopt))
            {
                ...
                if (option->size < sizeof(*eopt) || offset + option->size > sect->sh_size)
                {
...
process_gnu_liblist(Filedata *filedata)
{
         ...
         i++, section++)
    {
        ...
        case SHT_GNU_LIBLIST:
            ...
            if (strtab == NULL || section->sh_entsize != sizeof(Elf32_External_Lib))
            {
                free(elib);
                free(strtab);
                res = FALSE;
...
print_core_note(Elf_Internal_Note *pnote)
{
    ...
    if (count > ((bfd_vma)-1 - 2 * addr_size) / (3 * addr_size) || pnote->descsz < 2 * addr_size + count * 3 * addr_size)
    {
...
print_gnu_property_note(Filedata *filedata, Elf_Internal_Note *pnote)
{
    ...
    while (ptr < ptr_end)
    {
        ...
        {
            if (filedata->file_header.e_machine == EM_X86_64 || filedata->file_header.e_machine == EM_IAMCU || filedata->file_header.e_machine == EM_386)
            {
...
print_v850_note(Elf_Internal_Note *pnote)
{
    ...
    case V850_NOTE_ALIGNMENT:
        switch (val)
        {
        case EF_RH850_DATA_ALIGN4:
            printf(_("4-byte\n"));
            return TRUE;
        case EF_RH850_DATA_ALIGN8:
            printf(_("8-byte\n"));
            return TRUE;
        }
        break;
    ...
    case V850_NOTE_DATA_SIZE:
        switch (val)
        {
        case EF_RH850_DOUBLE32:
            printf(_("4-bytes\n"));
            return TRUE;
        case EF_RH850_DOUBLE64:
            printf(_("8-bytes\n"));
            return TRUE;
        }
        break;
...
print_ia64_vms_note(Elf_Internal_Note *pnote)
{
    ...
    case NT_VMS_LINKTIME:
        ...
        print_vms_time((bfd_int64_t)byte_get((unsigned char *)pnote->descdata, 8));
        printf("\n");
        break;
    ...
    case NT_VMS_ORIG_DYN:
        ...
        print_vms_time((bfd_int64_t)byte_get((unsigned char *)pnote->descdata + 8, 8));
        printf(_("\n   Link flags  : "));
        printf("0x%016" BFD_VMA_FMT "x\n",
                               ...
                               const char **pname)
{
    ...
    if (filedata->section_headers != NULL && (saved_filedata == NULL || filedata != saved_filedata))
    {
        Elf_Internal_Shdr *symsec;
    ...
    for (sym = symtab; sym < symtab + nsyms; sym++)
        if (sym->st_value == offset)
        {
            ...
            if (strtab[sym->st_name] == '$' && strtab[sym->st_name + 1] != 0 && strtab[sym->st_name + 2] == 0)
                continue;
            ...
            if (is_open_attr)
            {
                ...
                {
                case STB_GLOBAL:
                    if (saved_sym == NULL || ELF_ST_TYPE(saved_sym->st_info) != STT_OBJECT)
                        saved_sym = sym;
                    break;
             ...
             Filedata *filedata)
{
    ...
    if (((const_strneq(pnote->namedata, "GA") && strchr("*$!+", pnote->namedata[2]) != NULL) || strchr("*$!+", pnote->namedata[0]) != NULL) && (pnote->type == NT_GNU_BUILD_ATTRIBUTE_OPEN || pnote->type == NT_GNU_BUILD_ATTRIBUTE_FUNC))
    ...
    else if (((const_strneq(pnote->namedata, "GA") && strchr("*$!+", pnote->namedata[2]) != NULL) || strchr("*$!+", pnote->namedata[0]) != NULL) && (pnote->type == NT_GNU_BUILD_ATTRIBUTE_OPEN || pnote->type == NT_GNU_BUILD_ATTRIBUTE_FUNC))
                 ...
                 bfd_vma align)
{
    ...
    while ((char *)external < end)
    {
        ...
        if (!is_ia64_vms(filedata))
        {
            ...
            inote.descdata = ((char *)external + ELF_NOTE_DESC_OFFSET(inote.namesz, align));
            inote.descpos = offset + (inote.descdata - (char *)pnotes);
            next = ((char *)external + ELF_NOTE_NEXT_OFFSET(inote.namesz, inote.descsz, align));
        }
        else
        {
...
process_v850_notes(Filedata *filedata, bfd_vma offset, bfd_vma length)
{
    ...
    while ((char *)external + sizeof(Elf_External_Note) < end)
    {
        ...
        if (((char *)next > end) || ((char *)next < (char *)pnotes))
        {
        ...
        if (inote.namedata + inote.namesz > end || inote.namedata + inote.namesz < inote.namedata)
        {
...
process_note_sections(Filedata *filedata)
{
         ...
         i++, section++)
    {
        ...
        if ((filedata->file_header.e_machine == EM_V800 || filedata->file_header.e_machine == EM_V850 || filedata->file_header.e_machine == EM_CYGNUS_V850) && section->sh_type == SHT_RENESAS_INFO)
        {
...
void close_debug_file(void *data)
{
...
process_archive(Filedata *filedata, bfd_boolean is_thin_archive)
{
    ...
    if (do_archive_index)
    {
        ...
        if (!do_dynamic && !do_syms && !do_reloc && !do_unwind && !do_sections && !do_segments && !do_header && !do_dump && !do_version && !do_histogram && !do_debugging && !do_arch && !do_notes && !do_section_groups && !do_dyn_syms)
        {
    ...
    while (1)
    {
        ...
        if (is_thin_archive && arch.nested_member_origin == 0)
        {
            ...
            char *member_file_name = adjust_relative_path(filedata->file_name, name, namelen);
